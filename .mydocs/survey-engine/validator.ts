...
function validateConditionType(
  ...
  switch (questionType) {
    ...
    case 'likert':
      ...
      if (['greater', 'less', 'greaterOrEqual', 'lessOrEqual'].includes(operator)) {
...
function validateConditionOrder(
  ...
  const numericConditions = nextSteps
    .filter(n => n.condition && ['greaterOrEqual', 'greater'].includes(n.condition.operator))
    .map(n => ({
  ...
  for (let i = 1; i < numericConditions.length; i++) {
    if (numericConditions[i].value > numericConditions[i - 1].value) {
      warnings.push({
