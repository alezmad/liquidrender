...
packages/liquid-connect/    ← Query language + UVB
├── src/compiler/           ← LC parser, AST
├── src/semantic/           ← Semantic layer, registry
├── src/emitters/           ← SQL emitters (DuckDB, Postgres, Trino)
├── src/uvb/                ← Universal Vocabulary Builder
│   ├── models.ts           ← TypeScript types
│   ├── rules.ts            ← 7 hard rules engine
│   ├── extractor.ts        ← Schema extraction
│   └── adapters/           ← PostgreSQL adapter (more pending)
└── specs/                  ← UVB knowledge transfer docs
