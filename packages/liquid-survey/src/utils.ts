...
export function undo(history: SurveyHistory): { history: SurveyHistory; survey: GraphSurvey | null } {
  ...
  return {
    ...
    survey: cloneSurvey(state.survey)
  };
}
...
export function applyLocalization(
  ...
  for (const [nodeId, node] of Object.entries(localized.nodes)) {
    if (isQuestionContent(node.content)) {
      const content = node.content;
      const questionStr = strings[`${nodeId}.question`];
      if (questionStr) content.question = questionStr;
      const descStr = strings[`${nodeId}.description`];
      if (descStr) content.description = descStr;
      const placeholderStr = strings[`${nodeId}.placeholder`];
      if (placeholderStr) content.placeholder = placeholderStr;
      ...
      // Matrix rows/columns
      if (content.matrixRows) {
        content.matrixRows = content.matrixRows.map((row, i) =>
          strings[`${nodeId}.matrixRows.${i}`] || row
        );
      }
      if (content.matrixColumns) {
        content.matrixColumns = content.matrixColumns.map((col, i) =>
          strings[`${nodeId}.matrixColumns.${i}`] || col
        );
      }
      ...
      // Likert labels
      if (content.likertLabels) {
        const startStr = strings[`${nodeId}.likertLabels.start`];
        if (startStr) content.likertLabels.start = startStr;
        const endStr = strings[`${nodeId}.likertLabels.end`];
        if (endStr) content.likertLabels.end = endStr;
      }
    } else if (node.content && 'message' in node.content) {
      const content = node.content;
      const messageStr = strings[`${nodeId}.message`];
      if (messageStr) content.message = messageStr;
      const titleStr = strings[`${nodeId}.title`];
